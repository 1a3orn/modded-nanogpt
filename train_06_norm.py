...
class RMSNorm(nn.Module):
    def __init__(self, dim: int):
        super().__init__()
        self.scale = nn.Parameter(torch.full((dim,), 1e-5))
...
class CastedLinear(nn.Linear):
    ...
    def reset_parameters(self) -> None:
        ...
        with torch.no_grad():
            ...
            # Compensate for FP8 quantization by reducing initial weights
            if self.use_fp8:
                self.weight.mul_(0.67 * (self.in_features ** 0.5))
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, x0: Tensor, block_mask: BlockMask):
        ...
        if self.attn is not None:
            x = x + self.attn(self.norm1(x), ve, block_mask)
...
if master_process:
    ...
    print(logfile)
    wandb.init(project="nanogpt-base", config=args, experiment="06_norm")
