...
class ConvEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.conv_head = nn.Conv1d(
            ...
            groups = self.num_convs,
            padding=self.conv_length - 1,  # Causal padding to ensure no leakage from future tokens
        )
        self._init_weights()
    ...
    def _init_weights(self):
        ...
        #nn.init.zeros_(self.conv_head.bias)
        # Convert both weights and bias to bfloat16 to match input type
        #self.conv_head.weight.data = self.conv_head.weight.data.to(torch.bfloat16)
        #self.conv_head.bias.data = self.conv_head.bias.data.to(torch.bfloat16)
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        super().__init__()
        self.embed = ConvEmbedding(vocab_size, model_dim)
