...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, max_seq_len: int, vocab_size: int, layer_idx: int):
        super().__init__()
        # skip attention of blocks.7 (the 8th layer) by @YouJiacheng
        self.attn = CausalSelfAttention(dim, num_heads, max_seq_len) if layer_idx != 7 else None
        self.mlp = MLP(dim, vocab_size)
        self.lambdas = nn.Parameter(torch.tensor([1., 0.]))

    def forward(self, x: Tensor, ve: Tensor | None, x0: Tensor, block_mask: BlockMask, input_seq: Tensor):
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.blocks = nn.ModuleList([Block(model_dim, num_heads, max_seq_len, vocab_size, i) for i in range(num_layers)])
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        for i in range(len(self.blocks)):
            ...
            x = self.blocks[i](x, ve[i], x0, block_masks[i], input_seq)
            if i < n:
...
if master_process:
    ...
    rt = wandb.init(project="nanogpt_experiment_05", config=args, name="04_heron_quarter_lr_0.005_init_0.0")
    if rt is None:
...
embed_params = [p for n, p in model.named_parameters() if "embed" in n and "heron" not in n]
heron_embed_params = [p for n, p in model.named_parameters() if "embed" in n and "heron" in n]
...
adam_params = [
    ...
    dict(params=heron_embed_params, lr=0.005),
