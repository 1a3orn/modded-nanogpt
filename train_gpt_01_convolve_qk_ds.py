...
class FullTwoTapConv(nn.Module):
    ...
    def __init__(self, head_size: int):
        ...
        self.weight = nn.Parameter(torch.empty(head_size, head_size * 2))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: [b, t, nh, head_size]
        returns: [b, t, nh, head_size]
        """
        b, t, nh, hd = x.shape
        assert hd == self.head_size

        # Create shifted version with zero prepend (along time dimension)
        x_prev = torch.zeros_like(x)
        if t > 0:  # Handles t=0 case safely; no-op for t=1
            x_prev[:, 1:] = x[:, :-1]

        # Concatenate along channel dimension: [b, t, nh, 2 * hd]
        x_pair = torch.cat([x_prev, x], dim=-1)

        # Efficient matrix multiplication using tensor contraction
        # [b, t, nh, 2*hd] @ [2*hd, hd] -> [b, t, nh, hd]
        y = torch.einsum('btni,oi->btno', x_pair, self.weight)
